...
import {
	getRegionsInsideRange,
	positionIsInsideAspRegion,
	replaceCharacter,
} from "./region";
...
function getObjectMembersCode(
	objectsToAdd: CompletionItem[],
	objectName: string
): boolean {
	for (const symbol of [...currentDocSymbols, ...builtInSymbols]) {
		if (
			symbol.symbol.kind === SymbolKind.Class &&
			symbol.symbol.name.toLowerCase() === objectName.toLowerCase()
		) {
			for (const item of symbol.symbol.children) {
				...
				if (
					objectsToAdd.some(
						(i) => i.label === completion.label && i.kind == completion.kind
					)
				)
					continue;
...
function getDocumentationForSymbol(
	symbol: DocumentSymbol,
	parentName: string
): string {
	for (const aspSymbol of [...currentDocSymbols, ...builtInSymbols]) {
		if (
			aspSymbol.symbol.name !== symbol.name ||
			aspSymbol.parentName?.toLowerCase() !== parentName.toLowerCase()
		) {
			continue;
		}
...
async function provideCompletionItems(
	...
	// If in `<style>`,perform request forwarding
	if (
		isInsideStyleRegion(
			htmlLanguageService,
			doc.getText(),
		...
		return (await forwardCSS(doc, position, context)).items;
	}
	...
	if (dotTypedMatch && dotTypedMatch.length > 0) {
		...
		for (const symbol of [...currentDocSymbols, ...builtInSymbols]) {
			...
			if (
				!symbol.isTopLevel &&
				symbol.parentName.toLowerCase() !== scope?.toLowerCase()
			) {
				continue;
			}
	...
	function getScope(): string {
		...
		for (let index = position.line - 1; index >= 0; index--) {
			...
			if (
				currentLine.text[currentLine.firstNonWhitespaceCharacterIndex] === "'"
			) {
				continue;
			}
